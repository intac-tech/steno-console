(function () {
  ...
  CodeMirror.defineExtension('commentRange', function (isComment, from, to) {
    const cm = this; const curMode = CodeMirror.innerMode(cm.getMode(), cm.getTokenAt(from).state).mode;
  ...
  CodeMirror.defineExtension('autoFormatRange', function (from, to) {
    const cm = this;
    const outer = cm.getMode(); const text = cm.getRange(from, to).split('\n');
    const state = CodeMirror.copyState(outer, cm.getTokenAt(from).state);
    const tabSize = cm.getOption('tabSize');

    let out = ''; let lines = 0; let atSol = from.ch == 0;
    function newline () {
      out += '\n';
      atSol = true;
      ++lines;
    ...
    for (let i = 0; i < text.length; ++i) {
      ...
      while (!stream.eol()) {
        ...
        const style = outer.token(stream, state); const cur = stream.current();
